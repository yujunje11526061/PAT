...
对于序列中任一位置i，记到该位置为止，已知的包含i位元素的的LIS大小为dp[i].
遍历其前面的所有位置，对任意位置j，若有l[j]值不大于l[i]的值，则长度为max(dp[i], dp[j]+1)。因为dp[j]为到那位置且包含l[j]的LIS长度，只有满足l[j]<=l[i]，l[i]才能添到尾部进行扩展。
...
该算法的空间、时间复杂度均为O(mn)，经过优化后（如转化为LIS），空间复杂度可为O(n)，时间复杂度为O(nlogn)。
'''
# ===============================================================================
# LIS写法
